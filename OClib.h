...
typedef void(__stdcall* OnEventCallbackTgtInfo)(const double&, const double&, const unsigned char&);
typedef void(__stdcall* OnEventCallbackMissileInfo)(const double&, const double&);
typedef void(__stdcall* OnEventCallbackLauncherInfo)(const double&, const double&, const unsigned char&);
typedef void(__stdcall* OnEventCallbackRadarInfo)(const double&, const double&);
typedef void(__stdcall* OnEventCallbackAtsConnection)(const int&);
typedef void(__stdcall* OnEventCallbackLcsConnection)(const int&);
typedef void(__stdcall* OnEventCallbackMssConnection)(const int&);
typedef void(__stdcall* OnEventCallbackAdrsConnection)(const int&);

class OcInterface
{
	virtual void Run(void) = 0;
	virtual INT32 SendScenarioInfoSetReq(UINT32 missileSpeed, FLOAT64 atStartLon, FLOAT64 atStartLat, FLOAT64 atEndLon,
		FLOAT64 atEndLat, UINT32 atSpeed, FLOAT64 lcsX, FLOAT64 lcsY, FLOAT64 adrsX, FLOAT64 adrsY, UCHAR lcsMissileCnt) = 0;
...
private:
	...
	OnEventCallbackMissileInfo		m_callbackMissileInfo;
	OnEventCallbackLauncherInfo		m_callbackLaunchInfo;
	OnEventCallbackRadarInfo		m_callbackRadarInfo;
	OnEventCallbackAtsConnection	m_callbackAtsConnection;
	OnEventCallbackLcsConnection	m_callbackLcsConnection;
	OnEventCallbackMssConnection	m_callbackMssConnection;
	OnEventCallbackAdrsConnection	m_callbackAdrsConnection;
...
public:
	...
	INT32 SendScenarioInfoSetReq(UINT32 missileSpeed, FLOAT64 atStartLon, FLOAT64 atStartLat, FLOAT64 atEndLon, FLOAT64 atEndLat,
		UINT32 atSpeed, FLOAT64 lcsX, FLOAT64 lcsY, FLOAT64 adrsX, FLOAT64 adrsY, UCHAR lcsMissileCnt) override;
	INT32 SendSimulationStartStopReq(UINT32 setValue) override;
	INT32 SendFireCmd(void) override;
	INT32 SendTgtInfoToMss(FLOAT64 tgtX, FLOAT64 tgtY);
	INT32 SendShotResult(UINT32 nShotResult);

	void RegisterOnEventCallbackTgtInfo(OnEventCallbackTgtInfo newVal);
	void RegisterOnEventCallbackMissileInfo(OnEventCallbackMissileInfo newVal);
	void RegisterOnEventCallbackLauncherInfo(OnEventCallbackLauncherInfo newVal);
	void RegisterOnEventCallbackRadarInfo(OnEventCallbackRadarInfo newVal);
	void RegisterOnEventCallbackAtsConnection(OnEventCallbackAtsConnection newVal);
	void RegisterOnEventCallbackLcsConnection(OnEventCallbackLcsConnection newVal);
	void RegisterOnEventCallbackMssConnection(OnEventCallbackMssConnection newVal);
	void RegisterOnEventCallbackAdrsConnection(OnEventCallbackAdrsConnection newVal);
