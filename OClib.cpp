// OClib.cpp : 정적 라이브러리를 위한 함수를 정의합니다.
//
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            else
            {
                ...
                {
                case MSGID_MISSILE_INFO:
                    ST_MissileInfo stMissileInfo;
                    memset(&stMissileInfo, 0, sizeof(stMissileInfo));
                    memcpy(&stMissileInfo, m_OcMssServerRecvBuff, sizeof(stMissileInfo));
                    m_callbackMissileInfo(stMissileInfo.missileLongitude, stMissileInfo.missileLatitude);
                    break;
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            else
            {
                ...
                case MSGID_LCS_STATUS_INFO:
                    ST_LcsInfo stLcsInfo;
                    memset(&stLcsInfo, 0, sizeof(stLcsInfo));
                    memcpy(&stLcsInfo, m_OcLcsServerRecvBuff, sizeof(ST_LcsInfo));
                    m_callbackLaunchInfo(stLcsInfo.LcsEquipInfo.Location.longitude, stLcsInfo.LcsEquipInfo.Location.latitude, stLcsInfo.LaunchPadDetailInfo.loadedMissileCnt);
                    break;
        ...
        while (1)
        {
            ...
            else
            {
                ...
                case MSGID_ADRS_TARGET_INFO:
                    ...
                    m_callbackTgtInfo(stTgtInfo.TgtDetailInfo[0].tgtLongitude, stTgtInfo.TgtDetailInfo[0].tgtLatitude, stTgtInfo.TgtDetailInfo[0].reserved);
                    break;

                case MSGID_ADRS_STATUS_INFO:
                    ST_AdrsInfo stAdrsInfo;
                    memset(&stAdrsInfo, 0, sizeof(stAdrsInfo));
                    memcpy(&stAdrsInfo, m_OcAdrsServerRecvBuff, sizeof(ST_AdrsInfo));
                    m_callbackRadarInfo(stAdrsInfo.AdrsEquipInfo.Location.longitude, stAdrsInfo.AdrsEquipInfo.Location.latitude);
                    break;
...
INT32 OcCommManager::SendScenarioInfoSetReq(UINT32 missileSpeed, FLOAT64 atStartLon, FLOAT64 atStartLat, FLOAT64 atEndLon, FLOAT64 atEndLat, UINT32 atSpeed, FLOAT64 lcsX, FLOAT64 lcsY, FLOAT64 adrsX, FLOAT64 adrsY, UCHAR lcsMissileCnt)
{
    INT32 nByte = 0;
    INT32 nResult = 1;
    ...
    stScenarioSetReq.lcsLoadMissileCount = lcsMissileCnt;
...
INT32 OcCommManager::SendTgtInfoToMss(FLOAT64 tgtX, FLOAT64 tgtY)
{
    INT32 nByte = 0;
    INT32 nResult = 1;

    ST_TgtInfo stTgtInfo;
    memset(&stTgtInfo, 0, sizeof(ST_TgtInfo));

    stTgtInfo.MsgHeader.msgId = MSGID_ADRS_TARGET_INFO;
    stTgtInfo.MsgHeader.msgLen = sizeof(ST_TgtInfo);
    stTgtInfo.MsgHeader.srcId = 1;
    stTgtInfo.MsgHeader.dstId = 3;

    stTgtInfo.TgtDetailInfo[0].tgtLongitude = tgtX;
    stTgtInfo.TgtDetailInfo[0].tgtLatitude = tgtY;

    nByte = SendOcMssMsg(&stTgtInfo, sizeof(ST_TgtInfo));
    if (nByte <= 0)
        nResult = -1;
...
INT32 OcCommManager::SendShotResult(UINT32 nShotResult)
{
    INT32 nByte = 0;
    INT32 nResult = 1;

    ST_ShootResult stShootResult;
    memset(&stShootResult, 0, sizeof(ST_ShootResult));

    stShootResult.MsgHeader.msgId = MSGID_SHOOT_RESULT;
    stShootResult.MsgHeader.msgLen = sizeof(ST_ShootResult);
    stShootResult.MsgHeader.srcId = 1;
    stShootResult.shootResult = nShotResult;

    stShootResult.MsgHeader.dstId = 3;
    nByte = SendOcMssMsg(&stShootResult, sizeof(ST_ShootResult));
    if (nByte <= 0)
        nResult = -1;

    stShootResult.MsgHeader.dstId = 2;
    nByte = SendOcAtsMsg(&stShootResult, sizeof(ST_ShootResult));
    if (nByte <= 0)
        nResult = -1;
...
void OcCommManager::RegisterOnEventCallbackMissileInfo(OnEventCallbackMissileInfo newVal)
{
    m_callbackMissileInfo = newVal;
}


void OcCommManager::RegisterOnEventCallbackLauncherInfo(OnEventCallbackLauncherInfo newVal)
{
    m_callbackLaunchInfo = newVal;
}


void OcCommManager::RegisterOnEventCallbackRadarInfo(OnEventCallbackRadarInfo newVal)
{
    m_callbackRadarInfo = newVal;
}


void OcCommManager::RegisterOnEventCallbackAtsConnection(OnEventCallbackAtsConnection newVal)
{
    m_callbackAtsConnection = newVal;
}


void OcCommManager::RegisterOnEventCallbackLcsConnection(OnEventCallbackLcsConnection newVal)
{
    m_callbackLcsConnection = newVal;
}


void OcCommManager::RegisterOnEventCallbackMssConnection(OnEventCallbackMssConnection newVal)
{
    m_callbackMssConnection = newVal;
}


void OcCommManager::RegisterOnEventCallbackAdrsConnection(OnEventCallbackAdrsConnection newVal)
{
    m_callbackAdrsConnection = newVal;
}